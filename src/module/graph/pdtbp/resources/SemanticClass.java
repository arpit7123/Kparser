...
import static module.graph.pdtbp.utils.ArgumentChecker.silentCheckIsNonEmpty;
...
public class SemanticClass {
  ...
  public boolean match(SemanticClass other) {
    ...
    for (int i = 0; i < thisSemantics.length; ++i) {
      ...
      String thisSemantic = Util.extractSemantic(thisSemantics[i], 2);//Settings.SEMANTIC_LEVEL);
      if (thisSemantic != null && thisSemantic.length() > 0) {
        for (int j = 0; j < otherSemantics.length; ++j) {
          String otherSemantic = Util.extractSemantic(otherSemantics[j], 2);//Settings.SEMANTIC_LEVEL);
