...
//					String fileName = file.getName().replaceAll("\\.mrg", "");
	...
	public static ArrayList<String> anyTextToSpanGen(Tree pennTree, String inputText) throws IOException {
		ArrayList<String> spanList = new ArrayList<String>();

		String orgText = inputText;
		int index = 0;
		Tree root = pennTree;//tr.readTree();
		int treeNumber = 0;
		List<Tree> leaves = root.getLeaves();
		...
		for (Tree leaf : leaves) {
			...
			String key = "test.txt.ptree" + "," + treeNumber + "," + nodeNumber;
			word = word.trim().replaceAll("\\s+", "");
			word = word.replaceAll(",", "COMMA");
			spanList.add(key + "," + spanString + "," + word);
		}
